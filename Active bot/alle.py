...
async def apply(ctx, *, args=None):
    ...
    category = discord.utils.get(
        ctx.guild.categories, name="| 𝐎𝐟𝐟𝐢𝐜𝐞 𝐔𝐬𝐞")
    ...
    if Diretor_role in ctx.author.roles:
        ...
        Member_has_too_high_role.set_footer(
            text=f"Alle Group Applications | Apply Error Message • {time_main2}")
    ...
    question2_em.set_footer(
        text=f"Alle Group Applications | Question 2 • {time_main2}")
    ...
    question3_em.set_footer(
        text=f"Alle Group Applications | Question 3 • {time_main2}")
    ...
    question4_em.set_footer(
        text=f"Alle Group Applications | Question 4 • {time_main2}")
    ...
    if row_count > 0:
        ...
        you_are_blacklisted.set_footer(
            text=f"Alle Group Applications | Blacklist System • {time_main2}")
...
async def adde(ctx):
    if ctx.message.guild == None:
      ...
      def check(message):
        return message.author == ctx.author and message.channel == ctx.channel
      # collect data endpoints for EM and ED
      em = 'bert1912'
      ed = '[A.G] D3ADBEATZZ'
      global time_main2
